...
export function TweaksPanel({ title = 'Tweaks', noDeckControls = false, children }) {
  ...
  const clampToViewport = useCallback(() => {
    ...
    const newPos = {
      x: Math.min(maxRight, Math.max(PAD, posRef.current.x)),
      y: Math.min(maxBottom, Math.max(PAD, posRef.current.y)),
    };
  ...
  const onDragStart = (e) => {
    ...
    const move = (ev) => {
      ...
      posRef.current = newPos;
      panel.style.right = newPos.x + 'px';
      panel.style.bottom = newPos.y + 'px';
    };
    const up = () => {
      clampToViewport();
